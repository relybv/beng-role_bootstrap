...
# This script installs puppet 4.x and deploy the manifest using puppet apply -e "include role_bootstrap"
# Usage:
# <powershell>
# Set-ExecutionPolicy Unrestricted -Force
# icm $executioncontext.InvokeCommand.NewScriptBlock((New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/relybv/dirict-role_bootstrap/master/files/bootme.ps1')) -ArgumentList ("role_bootstrap")
#</powershell>


  param(
    [string]$role = "role_bootstrap"
  )

  $puppet_source = "https://github.com/relybv/dirict-role_bootstrap.git"
  ...
  if(!(Test-Path -Path "C:\ProgramData\PuppetLabs\code\modules\role_bootstrap" )){
    $clone_args = @("clone",$puppet_source,"C:\ProgramData\PuppetLabs\code\modules\role_bootstrap" )
  } else {
    $clone_args = @("pull")
    Set-Location -Path "C:\ProgramData\PuppetLabs\code\modules\role_bootstrap"
  }
